<template>
  <div
    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <!-- SUCCESS STATE: show masked email (not full email) -->
      <div v-if="isSubmitted" class="space-y-8">
        <div class="text-center">
          <div
            class="mx-auto w-16 h-16 bg-green-100 dark:bg-green-900/20 rounded-full flex items-center justify-center mb-6">
      ...
      <!-- FORM STATE -->
      <form v-else class="mt-8 space-y-6 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg"
...
async function handleSubmit() {
  ...
  try {
    ...
    // ✅ Neutral message (no email revealed)
    message.value = 'If an account exists for this email, you will receive a reset link.'
    isSubmitted.value = true
  } catch (err: any) {
    // Even if backend sends error, show neutral success message
    message.value = 'If an account exists for this email, you will receive a reset link.'
    isSubmitted.value = true
  } finally {
